package Exercicios;

// Importa a classe Scanner para permitir a leitura do teclado
import java.util.Scanner;

public class Exercicio2{

    public static void main(String[] args) {

        // Instancia o Scanner para capturar as entradas do utilizador
        Scanner entrada = new Scanner(System.in);

        // Solicita ao utilizador um número inteiro entre 1 e 12
        System.out.println("Digite um número de 1 a 12 para saber o mês:");
        int mes = entrada.nextInt();

        // Estrutura switch para mapear o número digitado ao nome do mês
        switch (mes) {
            case 1:
                System.out.println("Janeiro");
                break; // Interrompe a execução do switch após encontrar a opção
            case 2:
                System.out.println("Fevereiro");
                break;
            case 3:
                System.out.println("Março");
                break;
            case 4:
                System.out.println("Abril");
                break;
            case 5:
                System.out.println("Maio");
                break;
            case 6:
                System.out.println("Junho");
                break;
            case 7:
                System.out.println("Julho");
                break;
            case 8:
                System.out.println("Agosto");
                break;
            case 9:
                System.out.println("Setembro");
                break;
            case 10:
                System.out.println("Outubro");
                break;
            case 11:
                System.out.println("Novembro");
                break;
            case 12:
                System.out.println("Dezembro");
                break;
            default:
                // Trata o caso em que o número digitado não está entre 1 e 12
                System.out.println("Mês inválido! Digite um valor entre 1 e 12.");
                break;
        }

        // Fecha o recurso Scanner
        entrada.close();
    }
}